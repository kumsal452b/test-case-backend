    ...
    try {
      ...
      if (await this.investorModel.findOne({ user_id: investor.user_id })) {
        ...
        await this.investorModel.create({
          ...
          user_full_name: replaceTurkishCharacters(investor.user_full_name),
          site: site._id,
        });
        investment.user = await this.investorModel.findOne({
          user_id: investor.user_id,
      ...
      investment.team = new Types.ObjectId('651e463e9e7e22ccbc2f76ae');
      /*
      investment.team = createDepositDto.team
        ? new Types.ObjectId(createDepositDto.team)
        : null;
       */
      ...
      if (method) {
        ...
        const bankAccounts = await this.bankAccountModel
          .find({ paymentmethods: method._id })
